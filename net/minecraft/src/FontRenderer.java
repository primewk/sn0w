...
public class FontRenderer {
   ...
   public void drawCenteredString(FontRenderer var1, String var2, double var3, double var4, int var5) {
      var1.drawStringWithShadow(var2, var3 - (double)(var1.getStringWidth(var2) / 2), var4, var5);
   }

   public int drawStringWithShadow2(String par1Str, double par2, double par3, int par4) {
      return this.drawString2(par1Str, par2, par3, par4, true);
   }

   public int drawString2(String par1Str, double par2, double par3, int par4, boolean par5) {
      int var6;
      if (par5) {
         var6 = this.renderString2(par1Str, par2 + 1.0D, par3 + 1.0D, par4, true);
         var6 = Math.max(var6, this.renderString2(par1Str, par2, par3, par4, false));
      } else {
         var6 = this.renderString2(par1Str, par2, par3, par4, false);
      }

      return var6;
   }

   private int renderString2(String par1Str, double par2, double par3, int par4, boolean par5) {
      if (par1Str == null) {
         return 0;
      } else {
         if ((par4 & -67108864) == 0) {
         ...
         this.renderStringImpl(par1Str, par5);
   ...
   public void drawStringWithShadow(String s, double x, double y, int col) {
      this.renderString(s, x + 1.0D, y + 1.0D, col, true);
         ...
         while(j1 < as1.length) {
            ...
            for(; this.getStringWidth(s1) > col; s1 = s1.substring(k1)) {
               ...
               if (s1.substring(0, k1).trim().length() > 0) {
                  this.drawString(s1.substring(0, k1), (double)x, (double)y, lx);
                  y += this.charHeight;
               }
            }

            if (s1.trim().length() > 0) {
               this.drawString(s1, (double)x, (double)y, lx);
               y += this.charHeight;
            }
         }
      }
   ...
   private float getCharWidthFloat(char par1) {
      if (par1 == 167) {
         ...
         return par1 > 0 && var2 != -1 ? (float)this.charWidth : 0.0F;
      }
   }
