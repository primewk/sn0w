...
public class NetClientHandler extends NetHandler {
   ...
   public void handleChat(Packet3Chat var1) {
      ...
      if (ChatTime.instance.isEnabled() && var1.message.startsWith("<")) {
         DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm");
         LocalDateTime now = LocalDateTime.now();
         String time = "[" + dtf.format(now) + "]";
         this.mc.ingameGUI.addChatMessage(time + " " + var1.message);
      } else {
   ...
   public void handleHandshake(Packet2Handshake var1) {
      ...
      if (var1.username.equals("-")) {
         var10003 = this.mc.session;
         this.addToSendQueue(new Packet1Login(Session.username, Client.protocolver));
      } else {
         try {
            StringBuilder var10002 = new StringBuilder("http://www.minecraft.net/game/joinserver.jsp?user=");
            var10003 = this.mc.session;
            URL var2 = new URL(var10002.append(Session.username).append("&sessionId=").append(this.mc.session.sessionId).append("&serverId=").append(var1.username).toString());
